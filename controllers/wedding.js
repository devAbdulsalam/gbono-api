...
async function uploadVideoToYouTube(filePath, filename) {
    try {
        ...
        const res = await youtube.videos.insert({
            ...
            requestBody: {
                snippet: {
                    title: `Wedding - ${filename}`,
                    description: 'Uploaded from the gbono Weddings app',
                    tags: ['Wedding', 'gbono', 'upload'],
                },
                status: {
...
export const uploadImage = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Failed to fetch Weddings', error });
    }
};

// http://localhost:9000/api/v1/Weddings?page=1&limit=5&search=party&type=image&date=2025-07-20&all=true
...
export const getWeddings = async (req, res) => {
    try {
        ...
        // Only approved Weddings unless admin explicitly requests all
        if (!(all === 'true' && isAdmin)) {
        ...
        res
            .status(500)
            .json({ message: 'Failed to fetch Weddings', error: error.message });
    }
};
...
export const uploadVideo = async (req, res) => {
    ...
    try {
        ...
        const videoResponse = await youtube.videos.insert({
            ...
            requestBody: {
                snippet: {
                    title: caption || 'Wedding Video',
                    description: 'User-generated Wedding',
                },
                status: {
...
// 🎥 GET /api/Weddings/videos
export const getWeddingVideos = async (req, res) => {
    try {
        const videos = await Wedding.find({
            type: 'video',
            status: 'approved',
        }).sort({ createdAt: -1 });
...
export const uploadMultipleWeddings = async (req, res) => {
    try {
        ...
        for (const file of files) {
            ...
            if (ext.startsWith('image/')) {
                ...
                const saved = await Wedding.create({
                    userId,
                    type: 'image',
...
export const deleteImages = async (req, res) => {
    try {
        ...
        const images = await Wedding.find({ type: 'image' }).select(
            '_id cloudinaryId'
        );
        if (!images.length) {
            return res
        ...
        const { deletedCount } = await Wedding.deleteMany({ type: 'image' });
